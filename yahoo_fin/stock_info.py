...
def tickers_ftse100(include_company_data=False):
    ...
    table = pd.read_html(
        io.StringIO("https://en.wikipedia.org/wiki/FTSE_100_Index"),
...
def tickers_ftse250(include_company_data=False):
    ...
    table = pd.read_html(
        io.StringIO("https://en.wikipedia.org/wiki/FTSE_250_Index"),
        attrs={"id": "constituents"},
    )[0]
...
def get_income_statement(ticker, yearly=True):
    ...
    try:
        if yearly:
            ...
            temp = json_info["incomeStatementHistoryQuarterly"][
...
def get_analysts_info(ticker, headers={"User-agent": "Mozilla/5.0"}):
    ...
    tables = pd.read_html(
        io.StringIO(requests.get(analysts_site, headers=headers).text)
    )
...
def get_live_prices(ticker_list):
    ...
    data = resp.json()
    results = {
        result["symbol"]: result["regularMarketPrice"]
...
def get_day_most_active(count: int = 100):
    return _raw_get_daily_info(
        f"https://finance.yahoo.com/most-active?offset=0&count={count}"
    )


def get_day_gainers(count: int = 100):
    return _raw_get_daily_info(
        f"https://finance.yahoo.com/gainers?offset=0&count={count}"
    )


def get_day_losers(count: int = 100):
    return _raw_get_daily_info(
        f"https://finance.yahoo.com/losers?offset=0&count={count}"
    )
...
def get_top_crypto():
    ...
    df["% Change"] = df["% Change"].map(
        lambda x: float(str(x).strip("%").strip("+").replace(",", ""))
    )
    del df["52 Week Range"]
    del df["Day Chart"]
...
def get_dividends(
    ticker, start_date=None, end_date=None, index_as_date=True, headers=default_headers
):
    ...
    if (
        "events" not in data["chart"]["result"][0]
        or "dividends" not in data["chart"]["result"][0]["events"]
    ):
        return pd.DataFrame()
...
def get_splits(
    ...
    if "events" not in data["chart"]["result"][0]:
        raise AssertionError(
            "There is no data available on stock events, or none have occured"
        )
